...
export const InputBlock = block(({ type, name, textSize, hoverFill, fill, color, readonly, disabled, value }: Partial<TInput>) => {
  ...
  const classDisabled = clsx(baseClass,
    !disabled && defaultClass,
    disabled && `bg-b-disabled text-f-disabled border-1 border-bnt-dark`,
  )
  const classReadOnly = clsx(baseClass,
    !readonly && defaultClass,
    readonly && 'text-primary bg-transparent outline-1 outline-primary'
  )
  return <input type={type} name={name} readOnly={readonly} disabled={disabled} className={readonly && !disabled ? classReadOnly : classDisabled} value={value} />
})
